...
const SecuritySidebar = () => {
    ...
    return (
        <>
            {/* Sidebar Container */}
            <div className={`fixed top-0 left-0 h-full bg-gray-900 text-white shadow-xl transition-all duration-300 ${isOpen ? "w-64" : "w-20"} flex flex-col`}>
                ...
                <nav className="flex flex-col mt-4 space-y-2 px-2">
                    {menuItems.map((item, index) => (
                        <Link
                            key={index}
                            to={item.path}
                            className={`flex items-center p-3 rounded-lg transition-all text-white ${
                    ...
                    <button
                        ...
                        className="flex items-center p-3 w-full text-red-400 hover:bg-gray-800 rounded-lg transition-all"
                    >
